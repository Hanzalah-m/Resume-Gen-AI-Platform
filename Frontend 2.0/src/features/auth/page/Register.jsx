...
const Register = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    setError('')
    const result = await handleSendOtp(username, email, password)
    if (result.success) {
      setStep('otp')
    } else {
    ...
    return (
      <div className="w-full max-w-xl rounded-2xl md:rounded-4xl border border-[#285A48] bg-[#10221B]/95 p-5 md:p-8 shadow-[0_30px_80px_rgba(14,47,40,0.35)] backdrop-blur-xl">
        <div className="mb-4 md:mb-5 text-center">
          <p className="text-xs md:text-sm font-semibold uppercase tracking-[0.2em] md:tracking-[0.35em] text-[#B0E4CC]">Verify your email</p>
          <h1 className="mt-2 md:mt-3 text-2xl md:text-4xl font-semibold text-white">Enter OTP</h1>
          <p className="mt-2 md:mt-3 text-xs md:text-sm text-[#B0E4CC]/80">We sent a code to {email}. It expires in 5 minutes.</p>
        </div>
        ...
        <form className="space-y-3 md:space-y-2" onSubmit={handleOtpSubmit}>
          <label className="block">
            <span className="text-xs md:text-sm font-medium text-[#B0E4CC]">One-Time Password</span>
            <input
              type="text"
              name="otp"
              value={otp}
          ...
          <button
            ...
            disabled={loading}
            className="w-full rounded-2xl md:rounded-3xl border border-[#408A71] px-5 py-2.5 md:py-3 text-sm font-semibold text-[#B0E4CC] transition hover:bg-[#408A71]/10 disabled:cursor-not-allowed disabled:opacity-60"
          >
            Resend OTP
          </button>
        </form>
